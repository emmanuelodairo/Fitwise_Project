...
function WorkoutCard({ workout, onEdit, onDelete }) {
  ...
  return (
    ...
    >
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ fontWeight: 700, color: "var(--text)", fontSize: "14px", marginBottom: "2px", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
          {workout.exercise}
        </div>
...
function ProgressionChart({ workouts }) {
  ...
  const chartData = useMemo(() => {
    if (!selected) return [];
    return workouts
      .filter(w => w.exercise === selected && w.weight > 0)
